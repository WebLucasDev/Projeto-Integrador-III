...
if(isset($_POST['delete'])){
   ...
   if($verify_delete->rowCount() > 0){
      ...
      $success_msg[] = 'Reserva excluída';
   }else{
      $warning_msg[] = 'Reserva já excluída';
   }
...
<section class="grid">
   ...
   <div class="box">
      ...
      <p>entrada : <span><?= $fetch_bookings['check_in']; ?></span></p>
      <p>saída : <span><?= $fetch_bookings['check_out']; ?></span></p>
      <p>quartos : <span><?= $fetch_bookings['rooms']; ?></span></p>
      <p>adultos : <span><?= $fetch_bookings['adults']; ?></span></p>
      <p>crianças : <span><?= $fetch_bookings['childs']; ?></span></p>
      <form action="" method="POST">
         <input type="hidden" name="delete_id" value="<?= $fetch_bookings['booking_id']; ?>">
         <input type="submit" value="delete booking" onclick="return confirm('realmente deseja deletar essa reserva?');" name="delete" class="btn">
      </form>
   </div>
   <?php
      }
   ...
   <div class="box" style="text-align: center;">
      ...
      <a href="dashboard.php" class="btn">volte ao início</a>
   </div>
   <?php
      }
   ?>
